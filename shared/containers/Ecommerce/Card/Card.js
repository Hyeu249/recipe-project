...
const { addCard, editCard, deleteCard } = cardActions; // -----> redux actions

export default function Cards() {
  const dispatch = useDispatch();
  const { cards } = useSelector((state) => state.Cards); // -----> redux selector
  ...
  const columns = createColumns(
    editColumn, // -----> edit item
    (value) => dispatch(deleteCard(value)) // -----> delete item
  );
  ...
  function submitCard(card) {
    if (state.modalType === "edit") dispatch(editCard(state.selectedCard));
    if (state.modalType === "add") dispatch(addCard(state.selectedCard));
